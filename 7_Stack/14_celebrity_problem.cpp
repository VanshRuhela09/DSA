...
class Solution {
  public:
    ...
    int celebrity(vector<vector<int> >& mat) {
        ...
        while(i<j){
            if(mat[i][j]==1){  //if i knows j means i is not celebrity
                i++;
            }
            else if(mat[j][i]==1){ //if j knows i means j is not celebrity
                j--;
            }
            else{ // if both don't know each other that means they both are not celebrity
                i++;
                j--;
            }
        }
