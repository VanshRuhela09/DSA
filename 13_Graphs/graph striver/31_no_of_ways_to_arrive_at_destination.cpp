...
public:
    int countPaths(int n, vector<vector<int>>& roads) {
        ...
        while(!pq.empty()){
            auto f = pq.top();
            long long topDist = f.first;
            int topNode = f.second;
            pq.pop();
            for(auto it : adj[topNode]){
                if(topDist + it.second < dist[it.first]){
                    dist[it.first] = topDist + it.second;
                    ways[it.first] = ways[topNode];.
                    
                    pq.push({dist[it.first],it.first});
                }
                else if(topDist + it.second == dist[it.first]){
                    ways[it.first] = (ways[it.first] + ways[topNode])%(mode);
                }
            }
        }
        return ways[n-1];
